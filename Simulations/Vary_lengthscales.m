...
time=clock;
foldername=['LocalisationMCResultsLengthscales/Run',date,'-',num2str(time(4)),'-',num2str(time(5))];
mkdir(foldername);
...
figure; clf;
errorbar(lengthscales,mean(p_Endpoint_DR,2),std(p_Endpoint_DR'),'b','linewidth',2);
hold on;
errorbar(lengthscales,mean(p_Endpoint_EKF,2),std(p_Endpoint_EKF'),'k','linewidth',2);
...
for particle_index=1:particle_indices
    ...
    errorbar(lengthscales,mean(p_Endpoint_PF(:,:,particle_index),2),std(p_Endpoint_PF(:,:,particle_index)'),'Color',color,'linewidth',2);
end
xlabel('Length scale (m)','Interpreter','Latex','Fontsize',fontsize);
...
grid on;
set(gca,'xscale','log');
saveas(gcf,[foldername,'/LocPositionEstimationErrorsMCRepsLengthScales']);
saveas(gcf,[foldername,'/EPSs/LocPositionEstimationErrorsMCRepsLengthScales'],'epsc');
